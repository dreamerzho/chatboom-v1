...
def test_message_deduplication():
    ...
    try:
        ...
        test_messages = [
            {
                'message_id': '1234567890123',  # 移除 seq 字段，只保留 message_id
...
def test_chatlog_processor_deduplication():
    """测试 ChatLogProcessor 的去重功能"""
    logger.info("=== 开始测试 ChatLogProcessor 去重功能 ===")
    
    try:
        # 创建测试项目
        test_project = Project(
            project_name="ChatLogProcessor测试项目",
            description="用于测试ChatLogProcessor去重功能",
            status="active"
        )
        db.session.add(test_project)
        db.session.commit()
        ...
        # 创建处理器实例
        processor = ChatLogProcessor(project_id)
        
        # 处理聊天记录
        group_info = {'name': '测试群聊', 'type': '内部沟通群'}
        result = processor.process_and_deduplicate(mock_chatlog, group_info)
        
        logger.info(f"处理结果：消息 {len(result['chat_messages'])} 条，文件 {len(result['file_records'])} 条")
        
        # 验证处理结果
        if len(result['chat_messages']) == 3:  # 应该处理所有消息，去重在数据库层面
            logger.info("✅ ChatLogProcessor 去重测试通过！")
            return True
        else:
            logger.error("❌ ChatLogProcessor 去重测试失败！")
            return False
            
    except Exception as e:
        logger.error(f"ChatLogProcessor 去重测试异常: {e}")
        return False
    finally:
        # 清理测试数据
        try:
...
def main():
    ...
    try:
        ...
        with app.app_context():
            ...
            test_results.append(("ChatLogProcessor去重", test_chatlog_processor_deduplication()))
